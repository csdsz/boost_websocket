...
namespace BTool
{
    namespace BoostNet1_71
    {
        ...
        private:
            ...
            {
                if (ec) {
                    std::cout << "-------------------------------------handle_connect " << m_socket.get_status() << std::endl;
                    close();
                    return;
                }
                ...
                m_socket.next_layer().async_handshake(
                    ...
                    boost::beast::bind_front_handler(
                        ...
                        shared_from_this()));
                std::cout << "-------------------------------------async_handshake 第一步  " << m_socket.get_status() << std::endl;
            }

            void handle_handshake(const boost::beast::error_code& ec)
            {
                if (ec) {
                    std::cout << "-------------------------------------handle_handshake 异常进入 " << m_socket.get_status() << std::endl;
                    close();
                    return;
                }
                ...
                m_socket.async_handshake(m_connect_ip, m_hand_addr,
                    [&](const boost::beast::error_code& ec) {
                        std::cout << "-------------------------------------async_handshake 回调进入  " << m_socket.get_status() << std::endl;
                        if (ec) {
                            std::cout << "-------------------------------------async_handshake 异常进入" << std::endl;
                            close();
                            return;
                        }
                        handle_start(ec);
                    });
                std::cout << "-------------------------------------async_handshake 第二步  " << m_socket.get_status() << std::endl;
            }

            // 处理开始
            void handle_start(boost::beast::error_code ec) {
                if (ec || !m_atomic_switch.start()) {
                    std::cout << "-------------------------------------handle_start 异常进入" << m_socket.get_status() << std::endl;
                    close();
                    return;
                }
            ...
            void handle_read(const boost::system::error_code& error, size_t bytes_transferred) {
                if (error) {
                    std::cout << "-------------------------------------handle_read 异常进入" << m_socket.get_status() << std::endl;
                    shutdown();
                    return;
                }
            ...
            {
                if (ec) {
                    std::cout << "-------------------------------------handle_write 异常进入" << std::endl;
                    shutdown();
                    return;
                }
                if (m_atomic_switch.has_stoped()) {
            ...
            void close() {
                ...
                if (m_atomic_switch.load_init_flag()) {
                    std::cout << "-------------------------------------close 执行 " << m_socket.get_status() << std::endl;
                    m_socket.close(boost::beast::websocket::close_code::normal, ignored_ec);
                }
                else {
                    std::cout << "-------------------------------------close 不执行 " << m_socket.get_status() << std::endl;
                }
